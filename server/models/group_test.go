...
func testGroupToManyMessages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadMessages(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyUserGroups(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadUserGroups(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyAddOpMessages(t *testing.T) {
	var err error
...
var (
	groupDBTypes = map[string]string{`ID`: `integer`, `UUID`: `uuid`, `Name`: `character varying`, `CreatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
