...
func testConversationToManyEvents(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.ConversationID, a.ID)
	queries.Assign(&c.ConversationID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.ConversationID, b.ConversationID) {
			bFound = true
		}
		if queries.Equal(v.ConversationID, c.ConversationID) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadEvents(ctx, tx, false, (*[]*Conversation)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testConversationToManyAddOpEvents(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.ConversationID) {
			t.Error("foreign key was wrong value", a.ID, first.ConversationID)
		}
		if !queries.Equal(a.ID, second.ConversationID) {
...
func testConversationToManySetOpEvents(t *testing.T) {
	var err error
	...
	if !queries.IsValuerNil(b.ConversationID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.ConversationID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.ConversationID) {
		t.Error("foreign key was wrong value", a.ID, d.ConversationID)
	}
	if !queries.Equal(a.ID, e.ConversationID) {
		t.Error("foreign key was wrong value", a.ID, e.ConversationID)
	}

	if b.R.Conversation != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.Conversation != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.Conversation != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.Conversation != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
...
func testConversationToManyRemoveOpEvents(t *testing.T) {
	var err error
