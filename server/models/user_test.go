...
func testUserToManyUserGroups(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userGroupDBTypes, false, userGroupColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userGroupDBTypes, false, userGroupColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserGroups().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserGroups(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserGroups); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserGroups = nil
	if err = a.L.LoadUserGroups(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserGroups); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyUserWalks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserWalks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAuthorWalks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.AuthorWalks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAuthorWalks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorWalks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AuthorWalks = nil
	if err = a.L.LoadAuthorWalks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorWalks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserGroups(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e UserGroup

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserGroup{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userGroupDBTypes, false, strmangle.SetComplement(userGroupPrimaryKeyColumns, userGroupColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserGroup{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserGroups(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserGroups[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserGroups[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserGroups().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpUserWalks(t *testing.T) {
	var err error
...
func testUserToManyAddOpAuthorWalks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthorWalks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AuthorWalks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AuthorWalks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AuthorWalks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `UUID`: `uuid`, `Handle`: `USER-DEFINED`, `FirebaseUID`: `character varying`, `ProfilePicture`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `Bio`: `text`}
	_           = bytes.MinRead
)
