...
func testUserToManyRecipientEvents(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.RecipientEvents().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadRecipientEvents(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RecipientEvents); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RecipientEvents = nil
	if err = a.L.LoadRecipientEvents(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RecipientEvents); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpRecipientEvents(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddRecipientEvents(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.RecipientEvents[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RecipientEvents[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RecipientEvents().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
