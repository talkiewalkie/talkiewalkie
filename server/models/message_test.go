...
func testMessageToOneGroupUsingGroup(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := MessageSlice{&local}
	if err = local.L.LoadGroup(ctx, tx, false, (*[]*Message)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testMessageToOneSetOpGroupUsingGroup(t *testing.T) {
	var err error
...
var (
	messageDBTypes = map[string]string{`ID`: `integer`, `Text`: `text`, `AuthorID`: `integer`, `GroupID`: `integer`, `CreatedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
